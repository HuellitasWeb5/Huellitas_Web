...
<html>
    <head>
        <script>
            ...
            var fields = [
                "fecha", "identificacion", "nombre", "telefono", 
                "direccion", "residencia", "codigo", "nombreMascota", 
                "fechaNacimiento", "genero", "cuidadosEspeciales", "ocupacion", 
                "ninos", "habitantes", "responsables", "motivacion"
            ];
            ...
            var radios = ["tiempoLibre", "espacio", "compromiso", "otrasMascotas", "propietario"];
            for (var i = 0; i < radios.length; i++) {
                var checked = document.querySelector('input[name="' + radios[i] + '"]:checked');
                if (!checked) {
                    alert("Por favor, seleccione una opción para: " + radios[i]);
                    return false;
                }
            }
            return true;
    ...
    <body>
        ...
        <footer>
            <form method="post" action="formularioInformacion.jsp" onsubmit="return validarFormulario();">
                ...
                <label><input type="checkbox" name="horarioVisitaDias" value="Domingo"> Domingo</label>
